...
class StealCheckerMod(loader.Module):
    """Session Steller Checker: .чек (reply to file)"""
    strings = {'name': 'чекер'}
    async def чекcmd(self, message):
        ...
        if not len(check) == 0:
            ...
            return await message.reply(f"<b>❗️WARNING❗️</b> ❌SCAM MODULE❌\n\n" + s)
        else:
            return await message.reply(f"<b>Module is clear✅</b>")