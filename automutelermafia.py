...
#                                   былины  |   баку black |  баку   | true black | true  |   mafia russia
        if message.sender_id not in [606933972, 1044037207, 1050428643, 761250017, 468253535, 1634167847, 1471847332, 1520369962]:
            return
        ...
        if ('гнетущей' in message.raw_text.split()) or ('выдержал' in message.raw_text.split()):
            msgs = []
            ...
            #     return
                ...
                for usr in message.entities:
                    if hasattr(usr, 'user_id'):
                        uid = usr.user_id
                        await message.respond(f"!warn {str(uid)}  Лив с игры")